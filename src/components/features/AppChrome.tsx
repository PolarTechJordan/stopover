...
export default function AppChrome({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-full flex-col">
      <TouchZoomLock />
      {!isDashboard && <MobileAppHeader />}

      {!isDashboard && <header className="sticky top-0 z-40 hidden items-center justify-between border-b border-slate-200/60 bg-white/82 px-4 py-3 backdrop-blur-md sm:px-6 sm:py-4 md:flex">
        ...
        <div className="flex items-center gap-5">
          <nav className="hidden items-center gap-6 text-sm font-semibold text-slate-600 lg:flex">
            <Link href="/dashborad" className="transition-colors hover:text-primary">
